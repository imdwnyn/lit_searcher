# LitSearcher — Research Paper Assistant
...
LitSearcher uses [uv](https://github.com/astral-sh/uv) for dependency management.
...
| ---------------- | ----------------------------------------------------------------------- | --------------------------------------------------- |
| `OPENAI_API_KEY` | LLM inference (`gpt-4o-mini`) and embeddings (`text-embedding-3-small`) | [platform.openai.com](https://platform.openai.com/) |
...
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cost-Engineered Architecture** | Standardized all reasoning nodes, routers, synthesizers, and evaluators to `gpt-4o-mini` with `text-embedding-3-small`, cutting API operational expenses by over 90% without degrading task quality |
...
| **Session isolation**            | Each session gets its own Qdrant collection (`litsearcher_{session_id}`) and a separate LangGraph SQLite checkpointer thread — prevents cross-session data leakage                                  |
...
LitSearcher includes an automated RAG evaluation pipeline (`evaluate.py`) built on [DeepEval](https://github.com/confident-ai/deepeval).

### Metrics (Threshold: 0.7)

| **Metric** | **What It Measures** |
|---|---|
| **Contextual Precision** | Whether relevant context chunks are ranked higher by the retriever |
| **Contextual Recall** | Whether the retrieved context covers all ground-truth facts in the expected output |
| **Contextual Relevancy** | The ratio of directly relevant sentences in the retrieved context to background noise |
| **Answer Relevancy** | Whether the generated response directly answers the user's inquiry without extraneous tangents |
| **Faithfulness** | Whether claims in the output are strictly factual and grounded in the retrieved context (hallucination check) |

### Benchmark Results (OpenClaw Research Report)

Across 10 end-to-end synthetic benchmark test cases using `gpt-4o-mini`:  

- **Faithfulness**: **0.95** (90.0% pass rate) — strong factual grounding with minimal hallucination  
- **Contextual Precision**: **0.89** (90.0% pass rate) — effective vector ranking  
- **Contextual Recall**: **0.90** (90.0% pass rate) — comprehensive context retrieval  
- **Answer Relevancy**: **0.79** (80.0% pass rate) — high task alignment  
- **Execution Profile**: 54.69 seconds total run time at an operational cost of \~$0.04 USD  
...
- On first run, synthetic golden test cases are generated directly from the uploaded research documents using DeepEval's `Synthesizer` and cached locally to `goldens.json`.  
- Test cases are asynchronously executed against the LangGraph pipeline, evaluated across all 5 core metrics, and written out to `eval_results.json` with per-test scores, pass/fail status, and diagnostic reasons.  
- Subsequent runs automatically reload `goldens.json` to enable rapid regression testing.  